...
impl TextDocument {
    ...
    // Apply a change to the document. This also returns a tree sitter InputEdit
    // representing the change but only if this was an incremental change. If
    // it was a full update this returns None.
    pub fn update(
        &mut self,
        ...
        if let Some(range) = change.range {
            ...
            // Figure out the new end position for tree sitter.
            let new_end_byte = byte_begin + change.text.len();

            let new_end_line = range.start.line as usize + added_line_offsets.len();
            let new_end_character = self.position_at_line(new_end_line, new_end_byte);

            Some(InputEdit {
                start_byte: byte_begin,
                old_end_byte: byte_end,
                new_end_byte,
                start_position: TsPoint::new(
                    range.start.line as usize,
                    range.start.character as usize,
                ),
                old_end_position: TsPoint::new(
                    range.end.line as usize,
                    range.end.character as usize,
                ),
                new_end_position: TsPoint::new(new_end_line, new_end_character),
            })
        } else {
...
mod test {
    ...
    */
    ...
    #[test]
    fn several_incremental_content_changes_tree_sitter() {
        let mut document =
            TextDocument::new("function abc() {\n  console.log(\"hello, world!\");\n}".to_string());

        let edit = document.update(&TextDocumentContentChangeEvent {
            text: "defg".to_string(),
            range: Some(LspRange::new(
                LspPosition::new(0, 12),
                LspPosition::new(0, 12),
        ...
        assert_eq!(
            ...
            Some(tree_sitter::InputEdit {
                ...
                old_end_byte: 12,
                new_end_byte: 12 + 4,
                start_position: TsPoint::new(0, 12),
                old_end_position: TsPoint::new(0, 12),
                new_end_position: TsPoint::new(0, 12 + 4),
            })
        );
        ...
        assert_eq!(
            ...
            Some(tree_sitter::InputEdit {
                start_byte: 21 + 15,
                old_end_byte: 21 + 28,
                new_end_byte: 21 + 15 + 19,
                start_position: TsPoint::new(1, 15),
                old_end_position: TsPoint::new(1, 28),
                new_end_position: TsPoint::new(1, 15 + 19),
            })
        );
        ...
        assert_eq!(
            ...
            Some(tree_sitter::InputEdit {
                ...
                old_end_byte: 16,
                new_end_byte: 16 + 3,
                start_position: TsPoint::new(0, 16),
                old_end_position: TsPoint::new(0, 16),
                new_end_position: TsPoint::new(0, 16 + 3),
            })
        );
    }

    // TODO: Integrate tree_sitter into the above tests.
